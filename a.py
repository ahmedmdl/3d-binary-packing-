...
def insert(box, ptr):
        ...
        if not fits(box, child ) and rotatefit(box, child):
                ...
                tmp = child.right
                visited = child.c
                child.right = split(box, child)
                child.s = [0,0,0]
                child.right.right.right.right = tmp
                #print("done")
                return visited, child.Rotate
        """    
        # try another config first when box doesn't fit
        # check if ptr.left points to a node instance 
        if isinstance(child.left, Node):
            print("went left")
            visited, rot = insert_left(box, child)
            if visited is not False:
                parent.

                
        if isinstance(ptr.left.left, Node):
            print("went left left")
            visited, rot = insert_left_left(box, child)
            if visited is not False:
                parent.            
        ...
        if not fits(box, child):
            ...
            x, visited = Fit(remainder(box,child),adj(child, s),child,box,s)
            if x is not False:
                #print("**************************************")
                #pr(x)
                #print("**************************************")
                ptr = x
                return child.c, False
            #print("ss")
            visited = parent.c
            parent.right = split(box, parent)
            parent.s = [0,0,0]
...
def insert_left(box, ptr):              
            if fits(box, ptr.left):
                return insert(box, ptr.left)
            
            return False, False
    
def insert_left_left(box, ptr):
            if fits(box, ptr.left.left):
                return insert(box, ptr.left.left)
...
def Fit(rem_arr,
    ...
    for i in adj_arr:
        if( i[1] == "py" and arr_fits([rem_arr[0],rem_arr[1],rem_arr[2]],
            ...
            return assemble(fit_py(i[0],ptr,box), i[0], big_cont, ptr), i[0].c
            
        elif( i[1] == "ny" and arr_fits([rem_arr[0],rem_arr[1],rem_arr[2]],
                                        i[0])):
            
            return assemble(fit_ny(i[0],ptr,box), i[0],big_cont, ptr), i[0].c
            
        elif( i[1] == "px" and arr_fits([rem_arr[0],rem_arr[1],rem_arr[2]],
                                        i[0])):
            
            return assemble(fit_px(i[0],ptr,box), i[0],big_cont, ptr), i[0].c
            
        elif( i[1] == "nx" and arr_fits([rem_arr[0],rem_arr[1],rem_arr[2]],
                                        i[0])):
            
            return assemble(fit_nx(i[0],ptr,box), i[0],big_cont, ptr), i[0].c
            
        elif( i[1] == "pz" and arr_fits([rem_arr[0],rem_arr[1],rem_arr[2]],
                                        i[0])):
            
            return assemble(fit_pz(i[0],ptr,box), i[0],big_cont, ptr), i[0].c
            
        elif( i[1] == "nz" and arr_fits([rem_arr[0],rem_arr[1],rem_arr[2]],
                                        i[0])):
            
            return assemble(fit_nz(i[0],ptr,box), i[0],big_cont, ptr), i[0].c
        else:
            print("no fit")
            return False, False
